...
(async function start(adNumber) {
    ...
    if (adNumber === 2268163) {
        ...
        const mplhgjoPage = await SimulateMouse.selectTakeRandLinkClick(searchPageFinal, mplhgjo, redirect);
        const mplhgjoPageok = await redirect.toNextPage(mplhgjoPage);
        await mplhgjoPageok.waitFor(8000);
        await mplhgjoPageok.close();

        const mplhgjoPage2 = await SimulateMouse.selectTakeRandLinkClick(searchPageFinal, mplhgjo, redirect);

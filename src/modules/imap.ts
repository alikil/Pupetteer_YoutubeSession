...
const Gmail = async (userdata: { ahost: string, email: string; pass: string; }) => {
    const config = {
      imap: {
        user: `${userdata.email}`,
    ...
    const BodyTexts = results.map((res) => res.parts.filter((part) => part.which === "TEXT"));
...
Gmail(data).then((trg) => {console.log(trg); });
